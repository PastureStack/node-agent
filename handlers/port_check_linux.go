...
func hostSocketConflicts(requested []portCheckPort, dockerBindings map[string]bool) ([]portCheckConflict, bool) {
	return hostSocketConflictsAt("/host/proc", requested, dockerBindings)
}

func hostSocketConflictsAt(root string, requested []portCheckPort, dockerBindings map[string]bool) ([]portCheckConflict, bool) {
	netRoot := filepath.Join(root, "net")
	if info, err := os.Stat(netRoot); err != nil || !info.IsDir() {
		return nil, false
	}
	...
	for _, spec := range files {
		values, err := parseProcNet(filepath.Join(netRoot, spec.name), spec.protocol, spec.ipv6)
		if err != nil {
			complete = false
			continue
		}
