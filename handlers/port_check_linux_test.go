...
func TestHostSocketProbeFailsClosedWhenInventoryIsIncomplete(t *testing.T) {
	...
	previous := os.Getenv("CATTLE_HOST_PROC")
	if err := os.Setenv("CATTLE_HOST_PROC", root); err != nil {
		t.Fatal(err)
	}
	defer os.Setenv("CATTLE_HOST_PROC", previous)

	requested := []portCheckPort{{BindAddress: "0.0.0.0", PublicPort: 2201, Protocol: "tcp"}}
	conflicts, supported := hostSocketConflicts(requested, map[string]bool{})
	...
	_, supported = hostSocketConflicts(requested, map[string]bool{})
	if supported {
