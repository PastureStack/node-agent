...
func main() {
	driver := getenv("CONVOY_DRIVER_NAME", "convoytest")
	socket := getenv("CONVOY_SOCKET", fmt.Sprintf("/var/run/%s.sock", driver))
	root := getenv("CONVOY_DATA_DIR", fmt.Sprintf("/tmp/%s", driver))
