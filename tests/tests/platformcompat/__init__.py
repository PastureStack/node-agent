...
class Config:
    ...
    def _get_uuid_from_file(uuid_file):
        # This is a test fixture, not the production identity store. Keep
        # identities in memory so test-controlled paths cannot touch the host.
        return str(uuid4())
    ...
    def cadvisor_docker_root():
        from .plugins.docker import docker_client
